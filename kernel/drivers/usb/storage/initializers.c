...
int result = 0;
int act_len = 0;
	unsigned char cmd1[32] = {0x55, 0x53, 0x42, 0x43, 0x12, 0x34, 0x56, 0x78,
...
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
	if(b_usb_stor_err) {
		result = usb_stor_bulk_transfer_buf (us, us->send_bulk_pipe, cmd1, 31,
	&act_len);
		b_usb_stor_err = false;
		US_DEBUGP("usb_stor_huawei_scsi_init b_usb_stor_err b_usb_stor_err %d ,us->pusb_dev->descriptor.idProduct 0x%x\n",b_usb_stor_err,us->pusb_dev->descriptor.idProduct);
	} else {
	result = usb_stor_bulk_transfer_buf (us, us->send_bulk_pipe, cmd, 31,
	&act_len);
		US_DEBUGP("usb_stor_huawei_scsi_init b_usb_stor_err b_usb_stor_err %d \n",b_usb_stor_err);
	}
	US_DEBUGP("usb_stor_huawei_scsi_init performing result is %d, transfer the actual length=%d\n", result, act_len);
	return result;
}
