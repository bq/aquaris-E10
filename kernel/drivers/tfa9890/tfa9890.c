...
#ifdef I2C_BUS_R
static u8 *I2CDMABuf_r_va = NULL;
static u32 I2CDMABuf_r_pa = NULL;
#endif
#endif
#endif
...
#ifdef I2C_BUS_R
static struct tfa98xx_dev *tfa98xx_r;
#endif

/* tfa98xx I2C defination ++ */
#define TFA98XX_I2C_NAME   "i2c_smartpa"
#define TFA98XX_I2C_NAME_R   "i2c_smartpa_r"
#define TFA_I2CSLAVEBASE        0x34
#define TFA_I2CSLAVEBASE_R      0x35
#define I2C_STATIC_BUS_NUM        2

#define MAX_BUFFER_SIZE	1024 // 255
/* tfa98xx I2C defination -- */
...
#ifdef I2C_BUS_R
static struct i2c_board_info  tfa98xx_i2c_boardinfo_r = {
	I2C_BOARD_INFO(TFA98XX_I2C_NAME_R, TFA_I2CSLAVEBASE_R), 
};
#endif
...
static int tfa98xx_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	//int mode = 0;
	PRINT_LOG("tfa98xx_dev_ioctl cmd=%s(%d),arg=%s(%d)\n",cmd==I2C_SLAVE?"I2C_SLAVE":cmd==I2C_SLAVE_FORCE?"I2C_SLAVE_FORCE":"other:",cmd,
									arg==TFA_I2CSLAVEBASE?"TFA_I2CSLAVEBASE":arg==TFA_I2CSLAVEBASE_R?"TFA_I2CSLAVEBASE_R":"other:",arg);
	switch (cmd) 
	{
...
static ssize_t tfa98xx_r_dev_read(struct file *filp, char __user *buf, size_t count, loff_t *offset)
{
	struct tfa98xx_dev *tfa98xx_dev = filp->private_data;
	char tmp[MAX_BUFFER_SIZE];
	int ret;
    ...
    ret = tfa_i2c_read(tfa98xx_r->client, tmp, count);
    mutex_unlock(&tfa98xx_r->lock);
    tfa98xx_r->client->addr = tfa98xx_r->client->addr & I2C_MASK_FLAG;
    if (ret < 0) 
    {
        printk("%s: i2c_master_recv returned %d\n", __func__, ret);
        return ret;
	...
	/* Read data */
	mutex_lock(&tfa98xx_r->lock);
	ret = i2c_master_recv(tfa98xx_r->client, tmp, count);
	mutex_unlock(&tfa98xx_r->lock);
...
static ssize_t tfa98xx_r_dev_write(struct file *filp, const char __user *buf, size_t count, loff_t *offset)
{
	struct tfa98xx_dev  *tfa98xx_dev;
	char tmp[MAX_BUFFER_SIZE];
	int ret;
    ...
    //tfa98xx->client->addr = tfa98xx->client->addr & I2C_MASK_FLAG | I2C_DMA_FLAG | I2C_ENEXT_FLAG;
	mutex_lock(&tfa98xx_r->lock);
    ret = tfa_i2c_write(tfa98xx_r->client, tmp, count);
	mutex_unlock(&tfa98xx_r->lock);
    tfa98xx_r->client->addr = tfa98xx_r->client->addr & I2C_MASK_FLAG;
	if (ret < 0) 
	{
	...
	/* Write data */
	mutex_lock(&tfa98xx_r->lock);
	ret = i2c_master_send(tfa98xx_r->client, tmp, count);
	mutex_unlock(&tfa98xx_r->lock);
	if (ret < 0) 
	{
		printk("%s : i2c_master_send returned %d\n", __func__, ret);
...
static int tfa98xx_r_dev_open(struct inode *inode, struct file *filp)
{
...
static int tfa98xx_r_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct tfa98xx_dev *tfa98xx_dev = filp->private_data;
	//void __user *argp = (void __user *)arg;
	//int mode = 0;
	PRINT_LOG("tfa98xx_r_dev_ioctl cmd=%s(%d),arg=%s(%d)\n",cmd==I2C_SLAVE?"I2C_SLAVE":cmd==I2C_SLAVE_FORCE?"I2C_SLAVE_FORCE":"other:",cmd,
									arg==TFA_I2CSLAVEBASE?"TFA_I2CSLAVEBASE":arg==TFA_I2CSLAVEBASE_R?"TFA_I2CSLAVEBASE_R":"other:",arg);
	switch (cmd) 
	{
		/* This part do not work ok now , it is used for stereo application. */
...
static const struct file_operations tfa98xx_r_dev_fops = 
{
	.owner	= THIS_MODULE,
	.open	= tfa98xx_r_dev_open,
	.unlocked_ioctl = tfa98xx_r_dev_ioctl,
	.llseek	= no_llseek,
	.read	= tfa98xx_r_dev_read,
	.write	= tfa98xx_r_dev_write,
};



static int tfa98xx_r_i2c_probe(struct i2c_client *client,
				      const struct i2c_device_id *id)
{
	int ret = 0;
	int read_id = 0;
	...
	tfa98xx_r = kzalloc(sizeof(*tfa98xx_r), GFP_KERNEL);
	if (tfa98xx_r == NULL) 
	{
		dev_err(&client->dev,
				"failed to allocate memory for module data\n");
	...
	i2c_set_clientdata(client, tfa98xx_r);
	tfa98xx_r->client   = client;
	
	/* init mutex and queues */
	mutex_init(&tfa98xx_r->lock);

#ifdef I2C_USE_DMA
    	//msleep(50);
    
    	I2CDMABuf_r_va = (u8 *)dma_alloc_coherent(NULL, MAX_BUFFER_SIZE, &I2CDMABuf_r_pa, GFP_KERNEL);
    	if(!I2CDMABuf_r_va)
    	{
			printk("Allocate TFA98XX DMA I2C R Buffer failed!\n");
			return -1;
    	}
	...
	PRINT_LOG("tfa98xx_r_i2c_probe:rev_value=0x%x\n", rev_value);
	...
	tfa98xx_r->tfa98xx_device.minor = MISC_DYNAMIC_MINOR;
	tfa98xx_r->tfa98xx_device.name = TFA98XX_I2C_NAME_R;
	tfa98xx_r->tfa98xx_device.fops = &tfa98xx_r_dev_fops;

	ret = misc_register(&tfa98xx_r->tfa98xx_device);
	if (ret) 
	{
		printk("%s : misc_register failed\n", __FILE__);
		ret = -1;
...
err_misc_register:
	misc_deregister(&tfa98xx_r->tfa98xx_device);
i2c_error:
	mutex_destroy(&tfa98xx_r->lock);
	kfree(tfa98xx_r);
	tfa98xx_r = NULL;
err_exit:
	return ret;
...
static int tfa98xx_r_i2c_remove(struct i2c_client *client)
{
	struct tfa98xx_dev *tfa98xx_dev;
    ...
    if(I2CDMABuf_r_va)
    {
		dma_free_coherent(NULL, MAX_BUFFER_SIZE, I2CDMABuf_r_va, I2CDMABuf_r_pa);
		I2CDMABuf_r_va = NULL;
		I2CDMABuf_r_pa = 0;
    }
#endif
...
static void tfa98xx_r_i2c_shutdown(struct i2c_client *i2c)
{
	PRINT_LOG("Enter %s. +  %4d\n", __FUNCTION__, __LINE__);
}


static const struct of_device_id tfa98xx_r_of_match[] = {
	{ .compatible = "nxp,i2c_smartpa_r", },
	{},
};
MODULE_DEVICE_TABLE(of, tfa98xx_r_of_match);

static const struct i2c_device_id tfa98xx_r_i2c_id[] = {
	{ TFA98XX_I2C_NAME_R, 0 },
	{ }
};
MODULE_DEVICE_TABLE(i2c, tfa98xx_r_i2c_id);



static struct i2c_driver tfa98xx_r_i2c_driver = {
	.driver = {
		.name = TFA98XX_I2C_NAME_R,
		.owner = THIS_MODULE,
		.of_match_table = tfa98xx_r_of_match,
	},
	.probe =    tfa98xx_r_i2c_probe,
	.remove =   tfa98xx_r_i2c_remove,
	.id_table = tfa98xx_r_i2c_id,
	.shutdown = tfa98xx_r_i2c_shutdown,
};
...
#else
	//for mtk platform...
	ret = i2c_register_board_info(I2C_STATIC_BUS_NUM, &tfa98xx_i2c_boardinfo, 1 );
	if (ret < 0) {
...
#ifdef I2C_BUS_R
	ret = i2c_register_board_info(I2C_STATIC_BUS_NUM, &tfa98xx_i2c_boardinfo_r, 1 );
	if (ret < 0) {
		printk("i2c_register_board_info_r error %d\n", ret);
	}

	ret = i2c_add_driver(&tfa98xx_r_i2c_driver);
	if (ret != 0) {
	printk("Failed to register tfa98xx I2C R driver: %d\n", ret);
	}
#endif
