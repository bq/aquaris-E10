...
{	
	...
	mt_set_gpio_pull_enable(GPIO_MHL_RST_B_PIN,true);
...
void SiiMhlTxHwGpioSuspend(void)
{
    ...
    if(0)//(Tfa9890_IsFound())
    {
     for(i=0; i<ARRAY_SIZE(gpio_a); i++){
        ...
        mt_set_gpio_pull_select((gpio_a[i] | 0x80000000), GPIO_PULL_DOWN);
        mt_set_gpio_pull_enable((gpio_a[i] | 0x80000000), GPIO_PULL_ENABLE);
    }
    }
    else
    {
	for(i=0; i<ARRAY_SIZE(gpio_b); i++){
        ...
        mt_set_gpio_pull_select((gpio_b[i] | 0x80000000), GPIO_PULL_DOWN);
        mt_set_gpio_pull_enable((gpio_b[i] | 0x80000000), GPIO_PULL_ENABLE);
    }
...
void SiiMhlTxHwGpioSuspend(void)
{
    ...
    for(i=0; i<ARRAY_SIZE(gpio); i++){
        ...
        mt_set_gpio_pull_select((gpio[i] | 0x80000000), GPIO_PULL_DOWN);
        mt_set_gpio_pull_enable((gpio[i] | 0x80000000), GPIO_PULL_ENABLE);
    }
	hwPowerDown(MT6322_POWER_LDO_VM, "mhl_sii8338");
#endif
...
void SiiMhlTxHwGpioResume(void)
{
	...
	for(i=0; i<ARRAY_SIZE(gpio_rgb); i++){
		...
		mt_set_gpio_pull_enable((gpio_rgb[i]|0x80000000), GPIO_PULL_DISABLE);
	}	
	#if 0
	for(i=0; i<ARRAY_SIZE(gpio_i2s); i++){
		mt_set_gpio_mode((gpio_i2s[i]|0x80000000), GPIO_MODE_03);
		mt_set_gpio_pull_select((gpio_i2s[i]|0x80000000), GPIO_PULL_DOWN);
		mt_set_gpio_pull_enable((gpio_i2s[i]|0x80000000), GPIO_PULL_ENABLE);
	}
	#else
	for(i=0; i<ARRAY_SIZE(gpio_i2s2); i++){
		mt_set_gpio_mode((gpio_i2s2[i]|0x80000000), GPIO_MODE_03);
		mt_set_gpio_pull_select((gpio_i2s2[i]|0x80000000), GPIO_PULL_DOWN);
		mt_set_gpio_pull_enable((gpio_i2s2[i]|0x80000000), GPIO_PULL_ENABLE);
	}
	#endif
	#endif	
}
