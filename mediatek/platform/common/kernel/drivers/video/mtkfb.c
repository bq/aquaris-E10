...
extern int cur_hall_light_level;
#endif
...
#endif
//wangli_20141011 add to show lcm info
static ssize_t show_lcm_name(struct device *dev,struct device_attribute *attr, char *buf)
{
	ssize_t size;
	size=sprintf(buf,"%s\n",DISP_GetLCMId());
	return size;
...
static int mtkfb_probe(struct device *dev)
{
	...
	{
		s32 ret;
		ret = device_create_file(dev,&dev_attr_lcm_name);
	    if(ret)
	    {
	        printk("[lcm]device_create_file lcm_name fail! ret=%x\n",ret);
	    }
	}
...
static void mtkfb_early_suspend(struct early_suspend *h)
{
				#ifdef CKT_HALL_SWITCH_SUPPORT
				cur_hall_light_level = 0;
				#endif
    int i=0;
    MSG_FUNC_ENTER();
