...
status_t SpeechPhoneCallController::OpenModemSpeechDigitalPart(const modem_index_t modem_index, const audio_devices_t output_device)
{
    ...
    else // ADC/DAC I2S
    {
	 ...
	 ALOGD("tfa9890 [%s][L%d], Tfa9890_IsFound = %d,output_device=0x%x\n", __FUNCTION__, __LINE__, Tfa9890_IsFound(), output_device);
         if(Tfa9890_IsFound())
	{
...
status_t SpeechPhoneCallController::SetModemSpeechInterConnection(AudioDigitalType::Digital_Block block, modem_index_t modem_index, AudioDigitalType::InterConnectionState Connection)
{
    ...
    switch (block)
    {
	   ...
	   {
		case AudioDigitalType::I2S_OUT_2:
#if 0  //modify by he using for one spk when have a call
            mAudioDigitalInstance->SetinputConnection(Connection, modem_pcm_rx         , AudioDigitalType::O00); // MODEM_PCM_RX   -> 2nd_I2S_OUT_L
#endif
            mAudioDigitalInstance->SetinputConnection(Connection, modem_pcm_rx         , AudioDigitalType::O01); // MODEM_PCM_RX   -> 2nd_I2S_OUT_R
            break;
