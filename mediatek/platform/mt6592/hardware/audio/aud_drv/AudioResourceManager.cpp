...
status_t AudioResourceManager::TurnonAudioDevice(unsigned int mDlOutputDevice)
{
    ...
    if (DeviceCount == 1)
    {
	    ...
	    ALOGD("tfa9890 [%s][L%d], Tfa9890_IsFound = %d\n", __FUNCTION__, __LINE__, Tfa9890_IsFound());
	    if(Tfa9890_IsFound())
	    {
...
#if defined(NXP_SMARTPA_SUPPORT) || defined(USING_TFA9887_EXTAMP)|| defined(USING_TFA9890_EXTAMP)
	    ALOGD("tfa9890 [%s][L%d], Tfa9890_IsFound = %d\n", __FUNCTION__, __LINE__, Tfa9890_IsFound());
            if(Tfa9890_IsFound())
	    {
            mAudioAnalogInstance->AnalogOpen(AudioAnalogType::DEVICE_OUT_HEADSETR, AudioAnalogType::DEVICE_PLATFORM_MACHINE);
...
status_t AudioResourceManager::TurnonAudioDeviceIncall(unsigned int mDlOutputDevice)
{
    ...
    else
    {
	    ...
	    ALOGD("tfa9890 [%s][L%d], Tfa9890_IsFound = %d\n", __FUNCTION__, __LINE__, Tfa9890_IsFound());
            if(Tfa9890_IsFound())
	    {
            tfa9890_setSamplerate(16000);  //44100
            tfa9890_EQset(2);
            tfa9890_SpeakerOn();
	    }
#endif
...
status_t AudioResourceManager::StartInputDevice()
{
    ...
    if ((IsModeIncall()||(mAudioMode == AUDIO_MODE_IN_COMMUNICATION)) && (mDlOutputDevice & AUDIO_DEVICE_OUT_SPEAKER) && USE_REFMIC_IN_LOUDSPK) //ckt modify
    {
