...
#if defined(USING_TFA9890_EXTAMP) // add for fm issue alps01373308
...
#if defined(USING_TFA9887_EXTAMP)||defined(USING_TFA9890_EXTAMP) // add for fm issue alps01373308
...
status_t AudioFMResourceManager::SetFmDirectConnection(const bool enable)
{
        ...
        if(Tfa9890_IsFound())
	{
        ...
        // Set DAC I2S Out
#ifdef USING_TFA9890_EXTAMP // add for fm issue alps01373308
	ALOGD("tfa9890 [%s][L%d], Tfa9890_IsFound = %d\n", __FUNCTION__, __LINE__, Tfa9890_IsFound());
	if(!Tfa9890_IsFound())
	{
		if (AudioMTKStreamManager::getInstance()->IsOutPutStreamActive() == false)
