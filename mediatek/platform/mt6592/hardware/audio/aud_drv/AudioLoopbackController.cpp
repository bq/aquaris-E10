...
status_t AudioLoopbackController::OpenAudioLoopbackControlFlow(const audio_devices_t input_device, const audio_devices_t output_device, uint32 ul_samplerate, uint32 dl_samplerate)
{
    ...
    ALOGD("+%s(), bt_device_on = %d, mUseBtCodec = %d, mBtLoopbackWithoutCodec: %d, mBtLoopbackWithCodec: %d, AudioDigitalType=%d",
          __FUNCTION__, bt_device_on, mUseBtCodec, mBtLoopbackWithoutCodec, mBtLoopbackWithCodec,
          mAudioDigitalInstance->DlPolicyByDevice(output_device));
    ...
    else // ADC/DAC I2S
    {
        ...
        {
#if 0 //LiuHuojun 20140729 ¹¤³§Ä£Ê½MIC´µßäÐ¥½Ð,ÆÁ±Î¿¿½üMICµÄÀ®°È,Ö»ÔÊÐíÔ¶ÀëMICµÄÀ®°ÈÏì
		...
		mAudioDigitalInstance->SetHwDigitalGain(0x47FAC, AudioDigitalType::HW_DIGITAL_GAIN2);//-10db
