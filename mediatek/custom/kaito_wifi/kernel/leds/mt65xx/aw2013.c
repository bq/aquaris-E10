...
//*******************************AW2013ºôÎüµÆ³ÌÐò***********************************///
/*
void led_flash_aw2013( unsigned int id )   //id = 0/1/2£¬·Ö±ð¶ÔÓ\u0160LED0 LED1 LED2
{
	...
	hwctl_i2c_write_reg(0x01, 0x01);		// enable LED ²»Ê¹ÓÃÖÐ¶Ï			
	...
	hwctl_i2c_write_reg(0x39+id*3, Delay_time<<4 |Period_Num);	//led0  t_Delay=0s && cnt=ÎÞÇî\u017dó
					
	//read_reg(0x30,1,reg_buffer);
	//reg_data = reg_buffer[0] | (0x1 << id);
	hwctl_i2c_write_reg(0x30, 1<<id);	//led on	
		for (j=0; j < 50000; j++);//ÐèÑÓÊ±5usÒÔÉÏ
...
//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_AllOn
//µ÷ÓÃ\u017dËº¯Êý£¬Ôò10Â·µÆÈ«ÁÁ£¬Ã¿Â·µÄµçÁ÷·Ö±ðÓÉÈí\u0152þ¶ÀÁ¢¿ØÖÆ¡£
//-------------------------------------------------------------------------------------------
...
{
	AW2013_i2c_write_reg(0x12,0x00);   //OUT4~5ÅäÖÃÎªºôÎüµÆÄ£Ê\u0153Õâ\u0178äÈç¹ûÖ®Ç°ÉèÖÃ¹ý£¬¿ÉÒÔ²»Òª
	AW2013_i2c_write_reg(0x13,0x00);   //OUT0~3ÅäÖÃÎªºôÎüµÆÄ£Ê\u0153Õâ\u0178äÈç¹ûÖ®Ç°ÉèÖÃ¹ý£¬¿ÉÒÔ²»Òª
	AW2013_i2c_write_reg(0x04,0x03); 	 //OUT4-OUT5×ÔÖ÷ºôÎüBLINKÄ£Ê\u0153Ê¹Ä£?0ÎªblinkÄ£Ê\u0153£¬1ÎªFADEÄ£Ê\u0153
	AW2013_i2c_write_reg(0x05,0x0e);   //OUT0-OUT3×ÔÖ÷ºôÎüBLINKÄ£Ê\u0153Ê¹ÄÜ 0ÎªblinkÄ£Ê\u0153£¬1ÎªFADEÄ£Ê\u0153?	AW2013_i2c_write_reg(0x15,0x09);   //µ­\u0153øµ­³öÊ±\u0152äÉèÖÃÕâ\u0178äÈç¹ûÖ®Ç°ÉèÖÃ¹ý£¬¿ÉÒÔ²»Òª

	AW2013_i2c_write_reg(0x03,0x00);   //ÏÈ°Ñ03HÇå0
	AW2013_i2c_write_reg(0x03,0x01);   //\u017dÓ0Ð\u017dµ\u01531Ôò²úÉúµ­\u0153ø¹ý³Ì
	//AW2013_i2c_write_reg(0x03,0x00);   //\u017dÓ1Ð\u017dµ\u01530Ôò²úÉúµ­³öø¹ý³? 
}

//-------------------------------------------------------------------------------------------
//ÒÔÏÂÎªAW2013ÊµÏÖ\u0152\u017eÖÖÐ§¹ûµÄ²Î¿\u0152º¯Êý£¬ÓÃ»§Çë\u017eù\u0178Ý×Ô\u0152ºµÄÐ§¹ûÐèÒªÀ\u017dµ÷Õû¡£
//-------------------------------------------------------------------------------------------

//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_init_pattern
//µ÷ÓÃ\u017dËº¯Êý£¬Ôò6Â·µÆÈ«ÁÁ£¬Îª×ÔÖ÷ºôÎüBLINK Ä£Ê\u0153
//-------------------------------------------------------------------------------------------
void AW2013_init_pattern(void) 
{
 //AW2013»ù±\u0178µÄÐ§¹ûÊµÏÖ¡£6Â·×ÔÖ÷ºôÎü
	...
	// AW2013_OnOff(1);
	 ...
	 //Èç¹û²»²ÉÓÃ×ÔÖ÷ºôÎüÄ£Ê\u0153£¬Ôò²ÉÓÃÏÂÁÐ·\u0153Ê\u0153µ÷¹â
...
{  
	...
	hwctl_i2c_write_reg(0x37, Rise_time<<4 | Hold_time);	//led0  ÉÏÉýÊ±\u0152ä£¬±£³ÖÊ±\u0152äÉè¶\u0161							
	...
	hwctl_i2c_write_reg(0x3a, Rise_time<<4 | Hold_time);	//led1ÉÏÉýÊ±\u0152ä£¬±£³ÖÊ±\u0152äÉè¶\u0161								
	...
	hwctl_i2c_write_reg(0x3d, Rise_time<<4 | Hold_time);	//led2  ÉÏÉýÊ±\u0152ä£¬±£³ÖÊ±\u0152äÉè¶\u0161				
	...
	hwctl_delay_1us(8);//ÐèÑÓÊ±5usÒÔÉÏ
}
#else
void aw2013_breath_all(struct led_setting led0,struct led_setting led1,struct led_setting led2)  //led on=0x01   ledoff=0x00
{
    ...
    else
    {
    	...
    	hwctl_i2c_write_reg(0x37, Rise_time<<4 | blink);	//led0  ÉÏÉýÊ±\u0152ä£¬±£³ÖÊ±\u0152äÉè¶\u0161							
    	hwctl_i2c_write_reg(0x38, Fall_time<<4 | blink);	       //led0 ÏÂ\u0153µÊ±\u0152ä£¬¹Ø±ÕÊ±\u0152äÉè¶\u0161
    	hwctl_i2c_write_reg(0x39, Delay_time<<4| Period_Num);   //led0  ºôÎüÑÓ³ÙÊ±\u0152ä£¬ºôÎüÖÜÆÚÉè¶\u0161

    	hwctl_i2c_write_reg(0x3a, Rise_time<<4 | blink);	//led1ÉÏÉýÊ±\u0152ä£¬±£³ÖÊ±\u0152äÉè¶\u0161								
    	hwctl_i2c_write_reg(0x3b, Fall_time<<4 | blink);	       //led1 ÏÂ\u0153µÊ±\u0152ä£¬¹Ø±ÕÊ±\u0152äÉè¶\u0161
    	hwctl_i2c_write_reg(0x3c, Delay_time<<4| Period_Num);   //led1  ºôÎüÑÓ³ÙÊ±\u0152ä£¬ºôÎüÖÜÆÚÉè¶\u0161

    	hwctl_i2c_write_reg(0x3d, Rise_time<<4 | blink);	//led2  ÉÏÉýÊ±\u0152ä£¬±£³ÖÊ±\u0152äÉè¶\u0161				
...
//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_ComingCall
//À\u017dµçÊ±µÄÐ§¹û¡£6Â·×ÔÖ÷ºôÎü£¬ºôÎüÆµÂÊ\u0153Ï¿ì
//-------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_MissingCall
//ÓÐÎ\u017d\u0153ÓÀ\u017dµçÊ±µÄÐ§¹û¡£Ö»ÓÐ2 Â·×ÔÖ÷ºôÎü£¬ÆäËûµÆÃð¡£ºôÎüÆµÂÊ\u0153ÏÂý£¬²¢ÇÒÓÐÈ«°µÊ±\u0152ä
//-------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_ComingMsg
//À\u017d¶ÌÐÅÊ±µÄÐ§¹û¡£1Â·×ÔÖ÷ºôÎü£¬ºôÎüÆµÂÊ\u0153Ï¿ì
//-------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_MissingMsg
//ÓÐÎ\u017d\u0153Ó  ¶ÌÐÅÊ±µÄÐ§¹û¡£Ö»ÓÐÒ»Â·×ÔÖ÷ºôÎü£¬ºôÎüÆµÂÊ\u0153ÏÂý£¬²¢ÇÒÓÐÈ«°µÊ±\u0152ä
//-------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------
//º¯ÊýÃû: AW2013_GPIO_LED
//µ±ÓÃGPIO 6789À\u017d¿ØÖÆLEDµÄÑô\u0152«£¬ÔòÏÂÃæÕâ\u017eöº¯ÊýÄÜÊµÏÖ»ù±\u0178µÄºôÎü¹\u0160ÄÜ
//-------------------------------------------------------------------------------------------
void AW2013_GPIO_LED(void)
{
	...
	//ÓÃ»§¿É\u017eù\u0178ÝÐèÒªÀ\u017dµ÷ÕûÄ³\u0152\u017eÂ·Ñô\u0152«Îª\u017eß£¬»òÍ\u0161¹ýtimerÀ\u017dÑ­»·ÒÔ\u017dïµ\u0153ÏëÒªµÄÐ§¹û
	...
	AW2013_i2c_write_reg(0x02,0xff);   //OUT4~5Êä³ö\u017eßµçÆ\u0153¡£
	 ...
	 AW2013_i2c_write_reg(0x16,0x09);	 //È«ÁÁÈ«°µÊ±\u0152äÉèÖÃ 			 
	 AW2013_i2c_write_reg(0x14,0x3f);	 //×ÔÖ÷ºôÎüÊ¹ÄÜ 	 
	 AW2013_i2c_write_reg(0x11,0x92);     //¿ªÊ\u0152×ÔÖ÷ºôÎü¡£×¢Òâ	OUT6 ~9 ±ØÐëÅäÖÃÎªÍÆÍìÄ£Ê\u0153
}


//-------------------------------------------------------------------------------------------
//-----------AW2013 »ù±\u0178µÆÐ§º¯Êý¶\u0161Òå end----------------------------------------------
//-------------------------------------------------------------------------------------------
...
{	
	...
	AW2013_i2c_write_reg(0x20,0x3f);//OUT0¿Úµ÷¹â£¬µ÷¹âµÈ\u0152¶Îª0-255¡£OUT0~OUT5µÄµ÷¹âÖ\u017eÁîÒÀ\u017dÎÎª0x20~0x2f. Ð\u017d0¹Ø±Õ
	AW2013_i2c_write_reg(0x21,0x3f);
	AW2013_i2c_write_reg(0x22,0x3f);
	AW2013_i2c_write_reg(0x23,0x3f);
	AW2013_i2c_write_reg(0x24,0x3f);
	...
	AW2013_test();  //¶ÁÈ¡AW2013ÄÚ²¿\u0152Ä\u017dæÆ÷µÄÖµ£¬¿\u017dÓëÐ\u017d\u0153øÈ¥µÄÖµÊÇ·ñÒ»ÖÂ£¬ÓÉ\u017dËÅÐ¶ÏI2C\u0153Ó¿ÚÊÇ·ñÍ\u0161³©
}
