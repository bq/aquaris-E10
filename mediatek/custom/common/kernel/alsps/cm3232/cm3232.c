...
static int cm36283_i2c_command(struct i2c_client *client, unsigned int cmd, void *arg);

/*----------------------------------------------------------------------------*/
static const struct i2c_device_id cm36283_i2c_id[] = {{CM36283_DEV_NAME,0},{}};
static struct i2c_board_info __initdata i2c_cm36283={ I2C_BOARD_INFO(CM36283_DEV_NAME, 0x10)};
/*----------------------------------------------------------------------------*/
struct cm36283_priv {
	struct alsps_hw  *hw;
...
/*		
	databuf[0] = CM36283_REG_ALS_CONF;
			res = i2c_master_send(client, databuf, 0x20);
			if(res < 0)
			{
				APS_ERR("i2c_master_send function err\n");
...
long cm36283_read_als(struct i2c_client *client, u16 *data)
{
	...
	*data = ((databuf[1]<<8)|databuf[0]);
	APS_LOG("CM36283_REG_ALS_DATA value value:%d\n", *data);
	return 0;
	READ_ALS_EXIT_ERR:
	return res;
}
...
/*----------------------------------------------------------------------------*/
static int cm36283_ReadChipInfo(struct i2c_client *client, char *buf, int bufsize)
{
	u8 databuf[10];    
	...
	sprintf(buf, "cm3232 Chip");
	return 0;
}
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	struct i2c_client *client = cm36283_i2c_client;
	char strbuf[256];
	if(NULL == client)
	{
	...
	cm36283_ReadChipInfo(client, strbuf, 256);
