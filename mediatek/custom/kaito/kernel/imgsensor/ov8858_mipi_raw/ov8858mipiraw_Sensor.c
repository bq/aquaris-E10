...
//#define GN_MTK_BSP_LENS_COMPABILITY 1
...
kal_uint16 RG_Ratio_typical = 0x134; //RG_TYPICAL
kal_uint16 BG_Ratio_typical = 0x120;//BG_TYPICAL
...
{ 
	int otp_flag, addr, temp, i; //set 0x5002[3] to ¡°0¡± 
	int temp1; 

	temp1 = OV8858_read_cmos_sensor(0x5002); 
	OV8858_write_cmos_sensor(0x5002, (0x00 & 0x08) | (temp1 & (~0x08))); 
	// read OTP into buffer 
	OV8858_write_cmos_sensor(0x3d84, 0xC0);
	OV8858_write_cmos_sensor(0x3d88, 0x70); // OTP start address
	OV8858_write_cmos_sensor(0x3d89, 0x10);
	OV8858_write_cmos_sensor(0x3d8A, 0x72); // OTP end address
	OV8858_write_cmos_sensor(0x3d8B, 0x0a);
	OV8858_write_cmos_sensor(0x3d81, 0x01); // load otp into buffer
	mdelay(10);// OTP base information and WB calibration data

	otp_flag = OV8858_read_cmos_sensor(0x7010);
	addr = 0;
	...
	if(addr != 0) {
			...
			(*otp_ptr).module_integrator_id = OV8858_read_cmos_sensor(addr); 
			(*otp_ptr).lens_id = OV8858_read_cmos_sensor( addr + 1); 
			(*otp_ptr).production_year = OV8858_read_cmos_sensor( addr + 2); 
			(*otp_ptr).production_month = OV8858_read_cmos_sensor( addr + 3); 
			(*otp_ptr).production_day = OV8858_read_cmos_sensor(addr + 4);
			temp = OV8858_read_cmos_sensor(addr + 7); 
			(*otp_ptr).rg_ratio = (OV8858_read_cmos_sensor(addr + 5)<<2) + ((temp>>6) & 0x03); 
			(*otp_ptr).bg_ratio = (OV8858_read_cmos_sensor(addr + 6)<<2) + ((temp>>4) & 0x03); 
		}
	else { 
	...
	// OTP VCM Calibration 
	otp_flag = OV8858_read_cmos_sensor(0x7021); 
	addr = 0; 
	if((otp_flag & 0xc0) == 0x40) 
	{ 
	...
	if(addr != 0) 
		{ 
			(*otp_ptr).flag |= 0x20;
			temp = OV8858_read_cmos_sensor(addr + 2); 
			(* otp_ptr).VCM_start = (OV8858_read_cmos_sensor(addr)<<2) | ((temp>>6) & 0x03); 
			(* otp_ptr).VCM_end = (OV8858_read_cmos_sensor(addr + 1) << 2) | ((temp>>4) & 0x03); 
	...
	otp_flag = OV8858_read_cmos_sensor(0x7028); 
	addr = 0; 
	int checksum2=0; 
	if((otp_flag & 0xc0) == 0x40) {
	...
	if(addr != 0) 
	{
		for(i=0;i<240;i++) 
		{ 
			(* otp_ptr).lenc[i]=OV8858_read_cmos_sensor(addr + i); 
			checksum2 += (* otp_ptr).lenc[i]; 
		} 
		checksum2 = (checksum2)%255 +1; 
		(* otp_ptr).checksum = OV8858_read_cmos_sensor(addr + 240); 
	...
	{
		OV8858_write_cmos_sensor(i,0); // clear OTP buffer, recommended use continuous write to accelarate 
	} 
	//set 0x5002[3] to ¡°1¡± 
	temp1 = OV8858_read_cmos_sensor(0x5002); 
	OV8858_write_cmos_sensor(0x5002, (0x08 & 0x08) | (temp1 & (~0x08))); 
	return (*otp_ptr).flag; 
	} 
...
{ 
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		G_gain = 0x400 * G_gain / (Base_gain); // update sensor WB gain 
		if (R_gain>0x400) { 
			OV8858_write_cmos_sensor(0x5032, R_gain>>8); 
			OV8858_write_cmos_sensor(0x5033, R_gain & 0x00ff);
		}
		...
		if ((*otp_ptr).flag & 0x10) { 
			temp = OV8858_read_cmos_sensor(0x5000); 
			temp = 0x80 | temp; OV8858_write_cmos_sensor(0x5000, temp); 
			for(i=0;i<240;i++) { 
