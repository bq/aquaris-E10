...
my $fileForKernel_c = "kernel/drivers/mmc/core/custom_mmc.c";
my $fileForKernel_h = "kernel/drivers/mmc/core/custom_mmc.h";
&WriteFileForKernel($fileForKernel_c,$fileForKernel_h);
exit;
#****************************************************************************
# subroutine:  trim
...
#强行加入芯片THGBMAG7A2JBAIR,THGBMAG8A4JBA4R,以及mtk给的sandisk的芯片,由于92上面支持的是分离的emmc,所以不可以自动产生了
sub WriteFileForKernel
{
	my($fileForKernel_c,$fileForKernel_h)=@_;
	my $name="";
	my $num=$TotalCustemChips+1;
	print "===============\n\n\n";
          
	for(0.. $TotalCustemChips-1)
	{
#		$name.="\t$CustCS_PART_NUMBER[$_] $ID_String[$_]   $NAND_EMMC_ID[$_],\n";
#		print $name;
		
		my @nandid;
		my $temp=$NAND_EMMC_ID[$_];
		$temp=~s/^0x//;
		my @temp=split //,$temp;
...
AAA
	...
	open (FILE, ">$fileForKernel_c") or &error_handler("$fileForKernel_c: file error!", __FILE__, __LINE__);
	print FILE <<"__END_OF_FILE_C";
...
__END_OF_FILE_C
	...
	open (FILE, ">$fileForKernel_h") or &error_handler("$fileForKernel_c: file error!", __FILE__, __LINE__);
	print FILE <<"__END_OF_FILE_H";
